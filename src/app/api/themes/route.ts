...
import { getCustomThemes, saveCustomThemes, ThemeConflictError } from "@/lib/theme/store";
...
const THEME_TOKEN_KEYS = new Set<keyof ThemeSpec["colors"]>([
  ...
  "destructive", "destructive-foreground", "border", "input", "ring", "sidebar-background",
  "sidebar-foreground", "sidebar-primary", "sidebar-primary-foreground", "sidebar-accent",
  "sidebar-accent-foreground", "sidebar-border", "sidebar-ring", "display-accent", "display-red",
]);
...
function validateColors(value: unknown): ThemeSpec["colors"] | null {
  ...
  for (const [key, color] of Object.entries(value)) {
    if (!THEME_TOKEN_KEYS.has(key as keyof ThemeSpec["colors"]) || !isValidColorValue(key, color)) return null;
  }
...
function validateTheme(body: unknown): CustomTheme | null {
  ...
  return {
    id: t.id ?? "", name: t.name.trim(), builtIn: false, colors,
    fontSans: t.fontSans, fontDisplay: t.fontDisplay, radius: t.radius,
    cardStyle: t.cardStyle ?? "flat", buttonStyle: t.buttonStyle ?? "solid",
    canvasStyle: t.canvasStyle ?? "plain", headerStyle: t.headerStyle ?? "default",
  };
}
function slugify(name: string): string {
  const base = name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/đ/g, "d")
    .replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  return base || `theme-${Date.now()}`;
}
function conflictResponse() {
  return NextResponse.json(
    { error: "Giao diện đã được thay đổi bởi một yêu cầu khác", code: "THEME_CONFLICT" },
    { status: 409 }
  );
}
async function saveThemesOrConflict(next: CustomTheme[], expected: CustomTheme[]) {
  try {
    await saveCustomThemes(next, expected);
    return null;
  } catch (error) {
    if (error instanceof ThemeConflictError) return conflictResponse();
    throw error;
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Lỗi lấy danh sách giao diện", code: "INTERNAL_ERROR" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (customs.some((t) => t.id === id)) return NextResponse.json({ error: "Đã tồn tại giao diện trùng mã", code: "DUPLICATE_ID" }, { status: 409 });
    ...
    return NextResponse.json({ error: "Lỗi tạo giao diện", code: "INTERNAL_ERROR" }, { status: 500 });
  }
}
export async function PUT(request: Request) {
  ...
  try {
    const body = await request.json();
    const theme = validateTheme(body);
    if (!theme || !theme.id) return NextResponse.json({ error: "Thông tin giao diện không hợp lệ", code: "INVALID_THEME" }, { status: 400 });
    const customs = await getCustomThemes();
    const index = customs.findIndex((t) => t.id === theme.id);
    if (index === -1) return NextResponse.json({ error: "Không tìm thấy giao diện", code: "NOT_FOUND" }, { status: 404 });
    const updated: CustomTheme = { ...theme, id: theme.id, builtIn: false };
    const next = [...customs];
    next[index] = updated;
    const conflict = await saveThemesOrConflict(next, customs);
    if (conflict) return conflict;
    return NextResponse.json(updated);
  } catch (error) {
    logger.error("Update theme error:", error);
    return NextResponse.json({ error: "Lỗi cập nhật giao diện", code: "INTERNAL_ERROR" }, { status: 500 });
  }
}
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if (!id) return NextResponse.json({ error: "Thiếu mã giao diện", code: "MISSING_PARAMS" }, { status: 400 });
    const customs = await getCustomThemes();
    const next = customs.filter((t) => t.id !== id);
    if (next.length === customs.length) return NextResponse.json({ error: "Không tìm thấy giao diện", code: "NOT_FOUND" }, { status: 404 });
    const conflict = await saveThemesOrConflict(next, customs);
    if (conflict) return conflict;
    return NextResponse.json({ ok: true });
  } catch (error) {
    logger.error("Delete theme error:", error);
    return NextResponse.json({ error: "Lỗi xóa giao diện", code: "INTERNAL_ERROR" }, { status: 500 });
  }
}
