    ...
    try {
        ...
        if (!password || typeof password !== 'string') {
            return NextResponse.json(
                { error: 'Mật khẩu hiện tại là bắt buộc để bắt đầu đăng ký MFA', code: 'MISSING_CREDENTIALS' },
                { status: 400 }
            );
        }
        ...
        if (user.mfaEnabled) {
            return NextResponse.json(
                { error: 'MFA đã được kích hoạt cho tài khoản này', code: 'MFA_ALREADY_ENABLED' },
                { status: 400 }
            );
        }
        ...
        const qrUri = generateOtpAuthUri({
            secret,
        ...
        return NextResponse.json(
            { error: 'Đã xảy ra lỗi khi khởi tạo đăng ký MFA', code: 'SERVER_ERROR' },
            { status: 500 }
        );
    }
}
